...
import { Notebook, Plus, Trash2, Edit, Save, X, Calendar, Tag, Search } from 'lucide-react';
...
const AdvancedPersonalNotesApp = () => {
  ...
  const [newNote, setNewNote] = useState({ title: '', content: '', deadline: '', tags: [] });
  ...
  const handleAddNote = () => {
    if (newNote.title.trim() || newNote.content.trim()) {
      ...
      setNewNote({ title: '', content: '', deadline: '', tags: [] });
    }
  };
  ...
  return (
    <div className="max-w-full bg-slate-50 min-h-screen mx-auto p-4">
      <Card className="w-full">
        <CardHeader className="flex flex-row items-center justify-between">
          <div className="flex items-center space-x-2">
            <Notebook className="w-6 h-6" />
            <CardTitle>Advanced Personal Notes</CardTitle>
          </div>
          <div className="flex items-center space-x-2">
            <Input
                    ...
                    <Draggable key={note.id} draggableId={note.id.toString()} index={index}>
                      {(provided) => (
                        <div
                          ...
                          className="border rounded-lg p-4 relative hover:shadow-sm transition-shadow bg-white"
                        >
